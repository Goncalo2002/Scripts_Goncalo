...
function bombas_DisplayHelpText(str)
...
--[[Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(bombasTable)do
			id = v.id
			x = 1381.7392578125
			y = -2080.9526367188
			z = 51.998519897461
			bombaNome = "Bombas Gasolina da Amadora"
			bombaDono = v.bombaDono
			bombasPreco = 5000000
			payday = v.bmoney

			if(Vdist(pos.x, pos.y, pos.z, x, y, z) < 20.0)then
				if(bombaDono == "Nenhum")then
					DrawMarker(29, x, y, z-0.7, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.2001, 0, 255, 0, 255, 0, 0, 0, true)
					DrawText3D(x,y,z+0.23, "~y~"..bombaNome, 1.5)
					DrawText3D(x,y,z, "Um investimento do CARALHO", 0.7)
					DrawText3D(x,y,z-0.1, "~b~Dono: ~r~"..bombaDono.." ~b~Preço: ~g~$"..bombasPreco, 1)
					DrawText3D(x,y,z-0.21, "~b~Capital da Empresa: ~g~"..payday.."€!", 1)
				end
				if(Vdist(pos.x, pos.y, pos.z, x, y, z) < 1.5)then
						if (bombaDono == "Nenhum") then
							if (incircle == false) then
								bombas_DisplayHelpText("Pressiona ~INPUT_CONTEXT~ para comprares ~b~Bombas de Gasolina ~w~por ~g~$"..bombasPreco)
							end
							incircle = true
							if(IsControlJustReleased(1, 51))then
								TriggerServerEvent('comprarBombas', id)
							end
						end
